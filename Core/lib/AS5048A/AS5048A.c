...
uint16_t AS5048A_GetRaw() {
	...
	uint16_t result[1] = { 0 };
	HAL_StatusTypeDef status1 = HAL_SPI_TransmitReceive(spiDef, command1, result, 1, 10);
	CS_OFF();
	HAL_Delay(1);
	CS_ON();
	uint16_t command2[1] = { AS5048A_Read_NOP };
	HAL_StatusTypeDef status2 = HAL_SPI_TransmitReceive(spiDef, command2, result, 1, 10);
	CS_OFF();
